...
class PlanerFSonline_files(Screen, ConfigListScreen):
		...
		self.index=("",9)
		        ...
		        for x in conf_lines:
		            ...
		            fail=0
    			        ...
    			        #if len(split)>1 and not "name = url" in x:
    			                name= split[0].strip() 
	...
	def kn(self):
             if self["config"].getCurrent()[2] == 3:
                       ...
                       if en in self.onl_list2:      
                             ...
                             knt="1"
                             if en[4]=="1": knt="2"
                             #en= self.onl_list2(self.index)
                             #en2= (en[0],en[1],en[2],en[3],self["config"].getCurrent()[2].value)
	...
	def reloadList(self,num=None):
                ...
                for x in self.onl_list2:
                    try:
                      ...
                      #index = self.onl_list2.index(x)
                      #if x[3][4] != 
                      #self.onl_list2(index)=en2
                    except:
                       continue     
		self.list = list
	...
	def keySave(self):
            ...
            for x in self.onl_list2:
                ...
                if x[2]=="#":
                   try:
                       os.remove(self.cals_dir + str(x[0])+".ics")
                   except:
                      pass
