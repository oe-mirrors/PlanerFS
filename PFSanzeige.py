...
# ENIGMA IMPORTS
from enigma import eTimer, eDVBVolumecontrol, eServiceReference, eTimer, getDesktop, iPlayableService
...
conf = {"m_dauer": 0, "start_display_autohide": 0, "version": "", "cat_color_list": [], "fontsize_list": 20, "holidays_in_startscreen": "Yes", "countdown": None, "doubles": 0, "start_back": "1"}
...
class FehlerAnzeige(Screen, InfoBarNotifications):
	...
	def __init__(self, session, err_meld):
		...
		self["text1"] = Label(err_meld)  # (_("Missing / Mismatch in the event file"))
				...
				if doub:
					...
					if (x[6] != (0, 0) and d >= datetime.today()) or (x[9].date() >= date.today()):
						...
						for tmp in self.categories1:
							if (categories == tmp.strip().lower()) or (tmp.strip().lower() == such.lower()):  #.upper()
								jubi = tmp.strip()
						...
						if jubi:
							...
							if x[4] == 1:
								...
								y = (d_check, zeit + x[1] + alter2, color1, 4, color2)
								warnlist.append(y)
						else:
							y = (d_check, zeit + x[1], color1, 0) if x[4] == 1 else (d_check, zeit + x[1], color1, 4, color2)
							warnlist.append(y)
			else:
				break
